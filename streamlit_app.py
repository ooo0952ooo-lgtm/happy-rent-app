...
| 항목 | 검수 내용 | 판정 | 이유 |
| :--- | :--- | :--- | :--- |
| 여권 확인 | 첨부 사진이 실제 여권이 맞는지 확인 | [확인 / 불가] | (국적 정보 기재) |
| 로컬면허 확인 | 신분증 등이 아닌 실제 운전면허증이 맞는지 확인 | [확인 / 불가] | **(서류에 적힌 실제 발급일(ISSUE)을 YYYY-MM-DD 형식으로 반드시 포함하여 기재, 미제출이나 식별 불가 시 사유 기재)** |
| IDP 정식발급 여부 | 사설 기관 발급 여부 확인 (IAA 등 불가) | [확인 / 불가] | (발급 기관 정보 기재) |
| 성함 일치 여부 | 모든 서류 스펠링 대조 | [확인 / 불일치] | (일치 여부 및 영문 성함 기재) |
| 만 나이 | {today_date} 기준 만 26세 이상 | [확인 / 미달] | (생년월일 및 만 나이 기재) |
| 운전 경력 | 로컬/USFK 면허 취득 후 1년 경과 | [확인 / 불가] | **(로컬/USFK 면허의 실제 발급일을 바탕으로 경력 계산 근거 기재)** |
| 협약국 여부 | 제네바 또는 비엔나 협약국 리스트 포함 여부 | [확인 / 불가] | (해당 국가 기재) |
| IDP 유효기간 | 국제면허증 유효기간 만료 여부 | [확인 / 불가] | **(서류에 적힌 실제 발급일(ISSUE)과 만료일(EXPIRY)을 YYYY-MM-DD 형식으로 모두 반드시 포함하여 기재)** |
